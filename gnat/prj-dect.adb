   ...
   begin
      ...
      if Token = Tok_Identifier then
         ...
         --  Set, if appropriate the index case insensitivity flag
            ...
            elsif not Is_Allowed (Current_Attribute, In_Configuration) then
               if In_Configuration then
                  Error_Msg
                    ("attribute not allowed in configuration projects",
                     Token_Ptr);

               else
                  Error_Msg
                    ("attribute only allowed in configuration projects",
                     Token_Ptr);
               end if;
            end if;
   ...
   begin
      ...
      if Token = Tok_Identifier then
         ...
         if Current_Package  /= Empty_Package then
            ...
            if In_Configuration and then
              not Allowed_In_Configuration_Project (Current_Package)
            then
               Error_Msg
                 ("package """ &
                  Get_Name_String
                    (Name_Of (Package_Declaration, In_Tree)) &
                  """ is not allowed in configuration projects",
                  Token_Ptr);
            end if;
