...
package Prj.Attr is
   ...
   type Configuration is
     (Not_In_Conf_Projects,
      Allowed_In_Conf_Projects,
      Only_In_Conf_Projects);
   ...
   function Is_Allowed
     (Attribute : Attribute_Node_Id; In_Configuration : Boolean)
      return Boolean;
   --  Indicate if an attribute is allowed. Some attributes are only allowed
   --  in configuration projects, others are only allowed in user projects.
   ...
   --  Indicate if a package is allowed in a configuration project
...
private
   ...
   type Package_Record is record
      Name             : Name_Id;
      Known            : Boolean := True;
      In_Conf_Projects : Boolean := False;
