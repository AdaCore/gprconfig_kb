...
package Types is
   ...
   --  Type used to represent an empty or missing time stamp. Looks less than
   --  any real time stamp if two time stamps are compared. Note that although
   --  this is not a private type, clients should not rely on the exact way in
   --  which this string is represented, and instead should use the subprograms
   --  below.
