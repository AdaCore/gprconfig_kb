...
--     --  8-bit unsigned integer
   ...
   package Scans is
      ...
      --  These variables are initialized as required by
      --  Scn.Initialize_Scanner, and should not be referenced before such
      --  a call. However, there are situations in which these variables
      --  are saved and restored, and this may happen before the first
      --  Initialize_Scanner call, resulting in the assignment of invalid
      --  values. To avoid this, and allow building with the -gnatVa switch,
      --  we initialize some variables to known valid values.
      ...
      --  Within a type declaration, gives the location of the TYPE keyword
      --  that opened the type declaration. Used in checking the end column of
      --  a record declaration, which can line up either with the TYPE keyword,
      --  or with the start of the line containing the RECORD keyword.
      ...
      --  For identifiers, this is set to the Name_Id of the identifier
      --  scanned. For all other tokens, Token_Name is set to Error_Name. Note
      --  that it would be possible for the caller to extract this information
      --  from Token_Node. We set Token_Name separately for two reasons. First
      --  it allows a quicker test for a specific identifier. Second, it allows
      ...
      --  Valid only when Token = Tok_Integer_Literal, contains the value of
      --  the scanned literal.
